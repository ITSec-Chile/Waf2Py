...
def ExcludeLocal():
    ...
    if b == 'YES' and c == 'YES' and d == 'YES' and f == 'YES':
        ...
        if not data:
            ...
            replace = re.sub(r'^(##\w+Local\w+##\n).*(##\w+Local\w+##)', r'\1%s\2' %(rules_list), modsec_conf[0]['modsec_conf_data'], flags=re.S | re.M)
            db(db.production.id_rand == request.vars['id_rand']).update(modsec_conf_data=replace)#'\n'.join(r))
            db.commit()
            UpdateFiles = stuffs.CreateFiles()
            try:
                UpdateFiles.CreateModsecConf(modsec_conf[0]['app_name'], replace)
                a = stuffs.Nginx()
                b = a.Reload()
...
def ExcludeGlobal():
    ...
    if b == 'YES' and c == 'YES' and d == 'YES':
        ...
        if not data:
            ...
            replace = re.sub(r'^(##\w+Global\w+##\n).*(##\w+Global\w+##)', r'\1%s\2' %(rules_list), modsec_conf[0]['modsec_conf_data'], flags=re.S | re.M)
            db(db.production.id_rand == request.vars['id_rand']).update(modsec_conf_data=replace)#'\n'.join(r))
            db.commit()
            UpdateFiles = stuffs.CreateFiles()
            try:
...
def NginxLog():
    ...
    return dict(log_file=str(msg,'utf-8').splitlines(), page="NGINX logs", icon="fa fa-search", title="NGINX Logs")
...
def WafLogs():
    ...
    if b == 'YES' and n == False:
        ...
        query =  db(db.production.id_rand == request.args[0]).select(db.production.app_name)
        #Access Logs
        cmd = 'tac /opt/waf/nginx/var/log/'+query[0]['app_name']+'/'+query[0]['app_name']+'_access.log | head -1000'
        out1 = subprocess.Popen(cmd, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        #Error Logs
        cmd2 = 'tac /opt/waf/nginx/var/log/'+query[0]['app_name']+'/'+query[0]['app_name']+'_error.log | head -1000'
        ...
        #Modsec Json Logs
        cmd2 = 'tac /opt/waf/nginx/var/log/'+query[0]['app_name']+'/'+query[0]['app_name']+'_audit.log | head -1000'
        out2 = subprocess.Popen(cmd2, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg2, err2 = out2.communicate()
        #print err2
        modsec_list=[]
        ...
        #Debug Logs
        cmd3 = 'tac /opt/waf/nginx/var/log/'+query[0]['app_name']+'/'+query[0]['app_name']+'_debug.log | head -1000'
        ...
        summary_stats = db(db.summary.id_rand == request.args[0]).select().first()
    else:
        response.flash = 'Error in data supplied'
        redirect(URL('default','Websites'))
        id_rand = '#'

    return dict(page=query[0]['app_name']+" Logs", title="", icon="", summary=summary_stats,logs_file=logs_file, access_logs=access_list, error_logs=error_list, debug_logs=debug_list, id_rand=id_rand, modsec_json_logs=modsec_list)
...
def WafLogs_frame():
    ...
    if b == 'YES':
        ...
        cmd="tac /opt/waf/nginx/var/log/%s/%s_audit.log | head -1000" %(query[0]['app_name'], query[0]['app_name'])
        out1 = subprocess.Popen(cmd, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg = out1.communicate()[0]
        logs_list = []
        for i in str(msg, 'utf-8', errors='ignore').splitlines():
...
def DeleteRule():
    ...
    if b == 'YES' and c == 'YES' and request.vars['type'] == '0':
        ...
        UpdateFiles = stuffs.CreateFiles()
        try:
            UpdateFiles.CreateModsecConf(modsec[0]['app_name'], new_modsec[0]['modsec_conf_data'])
            stuffs.Nginx().Reload()
            #NewLogApp(db2, auth.user.username, "Mode: prod " +  data[0]['app_name'])
        except Exception as e:
            #NewLogError(db2, auth.user.username, "Mode: " + str(e))
...
def DownloadError():
    ...
    if b == 'YES':
        ...
        query = db(db.production.id_rand == request.args[0]).select(db.production.app_name, db.production.id_rand)
        out1 = subprocess.Popen(['/usr/bin/tar', '-czvf','/home/www-data/waf2py_community/applications/Waf2Py/static/logs/error_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz', '-C', '/opt/waf/nginx/var/log/'+query[0]['app_name']+'/',  query[0]['app_name']+'_error.log'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg,err = out1.communicate()
        r = A('Error_logs.tar.gz', _href='/Waf2Py/static/logs/error_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz')
    else:
...
def DownloadDebug():
    ...
    if b == 'YES':
        ...
        out1 = subprocess.Popen(['/usr/bin/tar', '-czf','/home/www-data/waf2py_community/applications/Waf2Py/static/logs/debug_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz', '-C', '/opt/waf/nginx/var/log/'+query[0]['app_name']+'/',  query[0]['app_name']+'_debug.log'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg,err = out1.communicate()
        r = A('Debug_logs.tar.gz', _href='/Waf2Py/static/logs/debug_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz')
    else:
        session.flash = 'Error in data supplied'
        r = ''
    return r
...
def DownloadModsecJson():
    ...
    if b == 'YES':
        ...
        out1 = subprocess.Popen(['/usr/bin/tar', '-czf','/home/www-data/waf2py_community/applications/Waf2Py/static/logs/modsec_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz', '-C', '/opt/waf/nginx/var/log/'+query[0]['app_name']+'/',  query[0]['app_name']+'_audit.log'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg,err = out1.communicate()
        r = A('Modsecurity_logs.tar.gz', _href='/Waf2Py/static/logs/modsec_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz')
    else:
        session.flash = 'Error in data supplied'
        r = ''
    return r
...
def DownloadAccess():
    ...
    if b == 'YES':
        random = randint(0, 999999999)
        query = db(db.production.id_rand == request.args[0]).select(db.production.app_name, db.production.id_rand)
        out1 = subprocess.Popen(['/usr/bin/tar', '-czvf','/home/www-data/waf2py_community/applications/Waf2Py/static/logs/access_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz', '-C', '/opt/waf/nginx/var/log/'+query[0]['app_name']+'/',  query[0]['app_name']+'_access.log'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        msg,err = out1.communicate()
        r = A('Error_logs.tar.gz', _href='/Waf2Py/static/logs/access_logs_'+query[0]['app_name']+'_'+str(random)+'.tar.gz')
    else:
        session.flash = 'Error in data supplied'
        r = ''
    return r
