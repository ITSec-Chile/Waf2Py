...
def AddVirtualIps():
    a = network.Network()
    return dict(ListIps=a.IpsUsed(), iface_names=a.iface_names(), page="Virtual IPs", icon="fa fa-pencil", title="Add Virtual IP")
...
def AddInterface():
    ...
    if ip == None and mask == None and request.vars['name'] in iface_names:
        ...
        if not query:
            try:
                c = n.AddIface(request.vars['ip'], request.vars['mask'], request.vars['name'], db)
